...
and (`PS_TZ_PT_ZHZXX_TBL`.`TZ_EFF_STATUS` = 'A'))) `TZ_RWZX_ZT_DESC`,
...
and (`PS_TZ_YJFSLSHI_TBL`.`TZ_FS_ZT` = 'FAIL'))) `TZ_SEND_FAIL_COUNT`,
(select count(1) from `PS_TZ_YJFSLSHI_TBL` 
where ((`PS_TZ_YJFSLSHI_TBL`.`TZ_EML_SMS_TASK_ID` = `A`.`TZ_EML_SMS_TASK_ID`) and 
(`PS_TZ_YJFSLSHI_TBL`.`TZ_FS_ZT` = 'RPT'))) `TZ_SEND_RPT_COUNT`,date_format(`A`.`TZ_RWTJ_DT`,'%Y-%m-%d %H:%i:%s') 
