...
public class TranzvisionMail {
	...
	public boolean openConnect() throws Exception {
		try {
			...
			// modify by caoy
			if (this.mailHost.endsWith(":465")) {
				System.out.println(hv);
				trustAllHttpsCertificates();  
				HttpsURLConnection.setDefaultHostnameVerifier(hv); 
				Security.addProvider(new com.sun.net.ssl.internal.ssl.Provider());
				
				this.props.put("mail.smtp.host", this.mailHost.substring(0, this.mailHost.length() - 4));
			...
			this.errDescription = ("Failed to open a connect to the mail server[" + this.mailHost + "," + this.userName
					+ "]:" + localException.getMessage());
			System.out.println(this.errDescription);
			this.sessionReady = false;
	...
	public boolean closeConnect() throws Exception {
		try {
			...
			this.errDescription = ("Failed to close the connect to the mail server[" + this.mailHost + "]:"
					+ localException.getMessage());
			writeLog(this.errDescription);
			localException.printStackTrace();
		}
	...
	public void addr1(String paramString) {
		...
		try {
			...
			this.addrCC = null;
			this.snum = 0;
			this.errDescription = ("Failed to set the cc email addresses for ecipients :"
					+ localException.getMessage());
			this.addrReady = false;
			writeLog(this.errDescription);
		}
	...
	public void addr2(String paramString) {
		...
		try {
			...
			this.addrBCC = null;
			this.tnum = 0;
			this.errDescription = ("Failed to set the bcc email addresses for ecipients :"
					+ localException.getMessage());
			this.addrReady = false;
			writeLog(this.errDescription);
		}
			...
			if (this.fromAddressAlias != null) {
				if (!this.fromAddressAlias.equals("")) {
					localInternetAddress1 = new InternetAddress(this.fromAddress,
							MimeUtility.encodeText(this.fromAddressAlias, "UTF-8", "B"));
				} else {
					localInternetAddress1 = new InternetAddress(this.fromAddress);
				}
			} else {
