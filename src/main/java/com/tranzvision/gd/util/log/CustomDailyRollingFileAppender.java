...
/**
 * 另存org.apache.log4j.DailyRollingFileAppender类修改自定义
 * 只保存过去30天的日志文件
 * @author zhanglang
 * 2019年6月17日
 *
...
public class CustomDailyRollingFileAppender extends FileAppender {
  ...
  */
  ...
  public CustomDailyRollingFileAppender (Layout layout, String filename,
    ...
    //this.datePattern = datePattern;	//注释，默认"'.'yyyy-MM-dd"格式按天滚动
    activateOptions();
  }
  ...
  public void activateOptions() {
    ...
    if(maxBackupIndex<=0) {  
        LogLog.error("maxBackupIndex reset to default value[30],orignal value is:" + maxBackupIndex);  
        maxBackupIndex = 30;  
    }  
  }
		...
		for (File ff : folder.listFiles()) { //遍历目录，将日期不在备份范围内的日志删掉
			if (ff.getName().startsWith(file.getName()) && !ff.getName().equals(file.getName())) {
				...
				if (result) {
					...
					LogLog.error("Failed to deleted old CustomDailyRollingFileAppender file :" + ff.getName());
				}
			}
		}
	}
