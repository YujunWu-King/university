...
public class TzInterviewExpExcelEngineCls extends BaseEngine{
	...
	public void OnExecute() throws Exception {
		...
		if (paramsMap != null) {
			...
			List<Map<String,Object>> itemsList = sqlQuery.queryForList(sqlExcelTpl, new Object[]{ expTmpId });
			
			List<String[]> arrExcelTplFieldList = new ArrayList<String[]>();
			for(Map<String,Object> itemMap : itemsList){
				...
				String strAppclsId = itemMap.get("TZ_APPCLS_ID") == null ? "" :  itemMap.get("TZ_APPCLS_ID").toString();
				if("".equals(filedId)){
					break;
				}
				if("".equals(filedName)){
				...
				dataCellKeys.add(new String[] { "Tranzvision-"+filedId, filedName });
				arrExcelTplFieldList.add(new String[] { filedId, filedName, filedSep, strAppclsId });
			}
			...
			for(String msPlanSeq : msPlanList){
				...
				try {
					appFormInfoView = sqlQuery.queryForObject(appFormInfoViewSQL,new Object[] { "TZ_FORM_VAL_REC" }, "String");
				} catch (Exception e) {
					appFormInfoView = "";
				}
				...
				try {
					exportExcelView = sqlQuery.queryForObject(exportExcelViewSQL,new Object[] { "TZ_FORM_EXL_REC" }, "String");
				} catch (Exception e) {
					exportExcelView = "";
				}
				...
				for(Map<String,Object> opridMap: opridList){
					...
					for(String [] arrExcelTplField : arrExcelTplFieldList){
						
						String strAppFormFieldValues = "";
						String strAppclsId = arrExcelTplField[3];
						if(strAppclsId==null || "".equals(strAppclsId) || "null".equals(strAppclsId)){
							...
							try {
								appFormFieldList = sqlQuery.queryForList(sqlAppFormField,new Object[] { appFormModalID, expTmpId, arrExcelTplField[0] });
							} catch (Exception e) {
							...
							if(appFormFieldList == null || appFormFieldList.size() == 0){
								...
								try {
									appFormFieldList = sqlQuery.queryForList(sqlAppFormField,new Object[] { appFormInfoView,expTmpId, arrExcelTplField[0] });
								} catch (Exception e) {
									appFormFieldList = null;
								}
							}
							...
							if(appFormFieldList == null || appFormFieldList.size() == 0){
								...
								try {
									appFormFieldList = sqlQuery.queryForList(sqlAppFormField,new Object[] { exportExcelView,expTmpId, arrExcelTplField[0] });
								} catch (Exception e) {
									appFormFieldList = null;
								}
							...
							if (appFormFieldList != null && itemsList.size() > 0) {
								for (int k = 0; k < appFormFieldList.size(); k++) {
									...
									if (isSingleValue) {
										...
										if (strSelectFieldBoolean == true) {
											try {
												strAppFormFieldValue = sqlQuery.queryForObject(sql1, new Object[] { strCodeTable,
																		Long.parseLong(appInsId), strAppFormField },"String");
											} catch (Exception e) {
											...
											try {
												strAppFormFieldValue = sqlQuery.queryForObject(sql1,
														new Object[] { Long.parseLong(appInsId), strAppFormField },
														"String");
												...
												if ("Select".equals(strComClassName) || "CompanyNature".equals(strComClassName)
													...
													try {
														strAppFormFieldValue = sqlQuery.queryForObject(
										...
										if (isDisplayRecordValue||isExportRecordValue) {
											...
											if (appFormInfoView != null && !"".equals(appFormInfoView)&&isDisplayRecordValue) {
												String sql3 = "SELECT " + strAppFormField + " FROM " + appFormInfoView
														+ " WHERE TZ_APP_INS_ID=?";
												try {
													strAppFormFieldValue = sqlQuery.queryForObject(sql3,
											...
											List<Map<String, Object>> list2 = sqlQuery.queryForList(sqlMutilValue,
													new Object[] { Long.parseLong(appInsId), strAppFormField });
									...
									if (strAppFormFieldValues == null || "".equals(strAppFormFieldValues)) {
										...
										if (arrExcelTplField[2] != null && !"".equals(arrExcelTplField[2])) {
											strAppFormFieldValues = strAppFormFieldValues + arrExcelTplField[2]
													+ strAppFormFieldValue;
										} else {
							...
							Map<String, Object> mapClsInfo = sqlQuery.queryForMap(strGetClsInfo,new Object[] {  strAppclsId });
							if(mapClsInfo != null){
								try{
							        ...
							        strAppFormFieldValues = (String) method.invoke(myMethod, appInsId,"",appFormModalID);
								}catch (Exception e) {
									e.printStackTrace();
								} 
							}
						...
						mapData.put("Tranzvision-"+arrExcelTplField[0], strAppFormFieldValues);
					}
