...
public class TzClpsExamineeExportEngineCls extends BaseEngine {
	...
	public void OnExecute() throws Exception {
		...
		try{
			...
			if(paramsMap != null){
				...
				///String sql = tZGDObject.getSQLText("SQL.TZMaterialInterviewReviewBundle.material.TzGetMaterialScoreItemInfo");
				String sql = "SELECT C.TZ_SCORE_ITEM_ID,C.DESCR,C.TZ_SCORE_ITEM_TYPE FROM PSTREENODE D,PS_TZ_CLASS_INF_T A,PS_TZ_MODAL_DT_TBL C,PS_TZ_RS_MODAL_TBL B WHERE D.TREE_NAME=B.TREE_NAME AND D.TREE_NODE=C.TZ_SCORE_ITEM_ID AND B.TZ_JG_ID=C.TZ_JG_ID AND B.TREE_NAME=C.TREE_NAME AND A.TZ_ZLPS_SCOR_MD_ID=B.TZ_SCORE_MODAL_ID AND B.TZ_JG_ID=A.TZ_JG_ID AND A.TZ_CLASS_ID=? ORDER BY D.TREE_NODE_NUM";
				...
				selfEmploymentXxxId = sqlQuery.queryForObject(hardcodeSql, new Object[]{ "TZ_BMB_ZZCYQC_XXX_ID" }, "String");
				...
				if(jacksonUtil.containsKey("appInsIds")){
					...
					if(listAppInsIdExport != null && listAppInsIdExport.size() > 0){
						for(String appInsId : listAppInsIdExport){
							...
							this.tzExportData(sqlQuery,classId,batchId,appInsId,listScoreItemField,dataList);
						}
					}
				}
	...
	private void tzExportData(SqlQuery sqlQuery,String classId,String batchId,String appinsId,List<String[]> listScoreItemField,List<Map<String, Object>> dataList){
		...
		if (mapExaminee != null) {
			...
			// 评委数据
			sql = "SELECT B.TZ_DLZH_ID,C.TZ_PYKS_XX,A.TZ_SCORE_INS_ID,A.TZ_KSH_PSPM FROM PS_TZ_CLPS_PW_TBL C,PS_TZ_AQ_YHXX_TBL B,PS_TZ_CP_PW_KS_TBL A WHERE C.TZ_CLASS_ID=A.TZ_CLASS_ID AND C.TZ_APPLY_PC_ID=A.TZ_APPLY_PC_ID AND C.TZ_PWEI_OPRID=A.TZ_PWEI_OPRID AND A.TZ_PWEI_OPRID=B.OPRID AND A.TZ_CLASS_ID=? AND A.TZ_APPLY_PC_ID=? AND A.TZ_APP_INS_ID=?";
			...
			for (Map<String, Object> mapJudge : listJudge) {
				...
				for (String[] scoreField : listScoreItemField) {
					...
					if ("D".equals(scoreItemType)) {
						...
						scoreValue = sqlQuery.queryForObject(sql, new Object[] { classId, scoreItemId, scoreValue }, "String");
					}
