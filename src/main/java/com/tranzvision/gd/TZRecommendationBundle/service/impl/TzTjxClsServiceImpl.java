...
public class TzTjxClsServiceImpl{
	...
	private SendSmsOrMalServiceImpl sendSmsOrMalServiceImpl;
	 ...
	 public String saveTJX(long numAppinsId,String strOprid, String strTjrId, String strEmail, String strTjxType, String strTitle, String strGname, String strName, String strCompany, String strPosition, String strPhone_area, String strPhone_no, String strGender, String strAdd1, String strAdd2, String strAdd3, String strAdd4, String strAdd5, String strTjrgx, String str_sysfilename, String str_filename, String str_refLetterType, String str_tjx_valid,String accessPath,String attAUrl){
		...
		String strRtn = "";
		String strTjxId = "";
		String strEmailflg = jdbcTemplate.queryForObject("select 'Y' from PS_TZ_KS_TJX_TBL A  where  A.TZ_APP_INS_ID=? AND A.TZ_TJR_ID<>? AND A.OPRID=? and TZ_MBA_TJX_YX='Y' and upper(A.TZ_EMAIL)=upper(?) limit 0,1", new Object[]{numAppinsId, strTjrId, strOprid, strEmail},"String");
		String str_language = jdbcTemplate.queryForObject("SELECT B.TZ_APP_TPL_LAN FROM PS_TZ_APP_INS_T A,PS_TZ_APPTPL_DY_T B WHERE A.TZ_APP_INS_ID=? AND A.TZ_APP_TPL_ID=B.TZ_APP_TPL_ID limit 0,1", new Object[]{numAppinsId},"String");
		if("Y".equals(strEmailflg)){
			//邮箱重复;
		    strRtn = messageTextServiceImpl.getMessageTextWithLanguageCd("TZGD_APPONLINE_MSGSET", "REF_E_DIF", str_language, "", "");
		}else{
			strTjxId = jdbcTemplate.queryForObject("select TZ_REF_LETTER_ID from PS_TZ_KS_TJX_TBL where TZ_APP_INS_ID=? and OPRID=?  and TZ_TJR_ID=? and TZ_MBA_TJX_YX='Y' limit 0,1", new Object[]{numAppinsId,strOprid,strTjrId},"String");;
			if((strTjxId == null || "".equals(strTjxId))
					&& "N".equals(str_tjx_valid)){
				b_flag = false;
			}
			...
			if(b_flag){
				if(strTjxId == null || "".equals(strTjxId)){
					...
					String str_seq2 = "00000000000000" + String.valueOf(getSeqNum.getSeqNum("TZ_KS_TJX_TBL", "TZ_REF_LETTER_ID"));
	 ...
	 public String changeTJR(long numAppinsId, String strTjrId, String strOprid){
		 ...
		 String str_language = "";
		 if(numAppinsId > 0 
				 && (strTjrId != null && !"".equals(strTjrId))
				 && (strOprid != null && !"".equals(strOprid))){
			 // 判断该推荐人的推荐信是否已提交;
			 String strTjxState = jdbcTemplate.queryForObject("select B.TZ_APP_FORM_STA from PS_TZ_KS_TJX_TBL A,PS_TZ_APP_INS_T B where A.TZ_APP_INS_ID=? AND A.TZ_TJR_ID=? AND A.OPRID=? AND A.TZ_MBA_TJX_YX='Y' AND A.TZ_TJX_APP_INS_ID=B.TZ_APP_INS_ID limit 0,1", new Object[]{numAppinsId, strTjrId, strOprid},"String");
			 str_language = jdbcTemplate.queryForObject("SELECT B.TZ_APP_TPL_LAN FROM PS_TZ_APP_INS_T A,PS_TZ_APPTPL_DY_T B WHERE A.TZ_APP_INS_ID=? AND A.TZ_APP_TPL_ID=B.TZ_APP_TPL_ID", new Object[]{numAppinsId},"String");
			 
			 if("U".equals(strTjxState)){
				 // 推荐信已提交;
				 strRtn = messageTextServiceImpl.getMessageTextWithLanguageCd("TZGD_APPONLINE_MSGSET", "REF_NO_C", str_language, "", "");
			 }else{
				 // 设置推荐信失效;
				 jdbcTemplate.update("update PS_TZ_KS_TJX_TBL A set TZ_MBA_TJX_YX='N' where  A.TZ_APP_INS_ID=? AND A.TZ_TJR_ID=? AND A.OPRID=?",new Object[]{numAppinsId, strTjrId, strOprid});
				 strRtn = "SUCCESS";
			 }
		 }else{
			 //参数不完整;
		     strRtn = messageTextServiceImpl.getMessageTextWithLanguageCd("TZGD_APPONLINE_MSGSET", "P_INCOM", str_language, "", "");
		 }
		 return strRtn;
	 ...
	 // 发送推荐信邮件;
	 public String sendTJX(long numAppinsId, String strOprid, String strTjrId){
		 String strRtn = "";
		 //String strMyName = "";
		 String str_opremail = "";
		 Map<String, Object> yhxxMap = jdbcTemplate.queryForMap("select TZ_REALNAME,TZ_EMAIL from PS_TZ_AQ_YHXX_TBL where OPRID=? limit 0,1", new Object[]{strOprid});
		 if(yhxxMap != null){
		 ...
		 String strEmail = "";
		 String strTjxType = "";
		 Map<String, Object> ksTjxMap = jdbcTemplate.queryForMap("select TZ_REF_LETTER_ID,TZ_TJX_TITLE,TZ_REFERRER_NAME,TZ_REFERRER_GNAME,TZ_GENDER,TZ_EMAIL,TZ_TJX_TYPE from PS_TZ_KS_TJX_TBL where TZ_APP_INS_ID=? and TZ_TJR_ID=? and OPRID=? and TZ_MBA_TJX_YX='Y'",new Object[]{numAppinsId, strTjrId, strOprid});
		 if(ksTjxMap != null){
		...
		String strExistFlg = "";
		Map<String, Object> map = jdbcTemplate.queryForMap("select 'Y' TZ_EXIST,TZ_JG_ID,TZ_CHN_MODAL_ID,TZ_ENG_MODAL_ID,TZ_CH_M_CONTENT,TZ_EN_M_CONTENT from PS_TZ_APP_INS_T A,PS_TZ_APPTPL_DY_T B WHERE A.TZ_APP_TPL_ID=B.TZ_APP_TPL_ID AND A.TZ_APP_INS_ID=? LIMIT 0,1",new Object[]{numAppinsId});
		if(map != null){
		...
		if(str_opremail != null && str_opremail.equals(strEmail)){
			mess = messageTextServiceImpl.getMessageTextWithLanguageCd("TZGD_APPONLINE_MSGSET", "SAME_EMAIL", str_language, "", "");
		    return mess;
		}
