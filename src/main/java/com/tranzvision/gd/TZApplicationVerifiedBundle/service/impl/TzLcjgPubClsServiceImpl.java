...
/**
 * PS:TZ_LCJG_PUB_PKG:TZ_LCJG_PUB_CLS
 * 
 * @author tang
 *
 */
@Service("com.tranzvision.gd.TZApplicationVerifiedBundle.service.impl.TzLcjgPubClsServiceImpl")
public class TzLcjgPubClsServiceImpl extends FrameworkImpl {
	@Autowired
	...
	public String tzQuery(String strParams, String[] errMsg) {
		...
		try {
			...
			String str_color_id = "", str_fb_desc = "",str_fb_desc1 = "",str_color="";
			...
			if (map != null) {
				...
				if (str_color_id == null || "".equals(str_color_id)) {
					sql = "SELECT TZ_CLS_RESULT,TZ_APPPRO_CONTENT,TZ_APPPRO_HF_BH FROM PS_TZ_CLS_BMLCHF_T WHERE TZ_CLASS_ID=? AND TZ_APPPRO_ID=? AND TZ_WFB_DEFALT_BZ='on' limit 0,1";
					...
					if (map1 != null) {
						...
						str_color_id = (String)map1.get("TZ_APPPRO_HF_BH");
					}
				}
				...
				str_color = jdbcTemplate.queryForObject("SELECT TZ_APPPRO_COLOR FROM PS_TZ_CLS_BMLCHF_T WHERE TZ_CLASS_ID=? AND TZ_APPPRO_ID=? AND TZ_APPPRO_HF_BH=?", new Object[]{str_bj_id, str_bmlc_id, str_color_id},"String");
				if(str_color == null){
					str_color = "";
				}
			}
			String str_lcname = jdbcTemplate.queryForObject("SELECT TZ_APPPRO_NAME FROM PS_TZ_CLS_BMLC_T WHERE TZ_CLASS_ID=? AND TZ_APPPRO_ID=?", new Object[]{str_bj_id, str_bmlc_id},"String");
			if(str_lcname == null){
				str_lcname = "";
			}
			...
			jsonMap.put("bmlc_color",str_color );
			jsonMap.put("ry_lc", str_lcname);
