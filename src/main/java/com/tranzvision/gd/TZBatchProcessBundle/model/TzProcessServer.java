package com.tranzvision.gd.TZBatchProcessBundle.model;

import java.util.Date;

public class TzProcessServer extends TzProcessServerKey {
    private String tzFwqIp;

    private String tzJcfwqMs;

    private String tzCzxtLx  ;

    private Integer tzRwxhJg;

    private Integer tzZdbxRws;

    private String tzBeizhu;

    private String tzYxzt;

    private Date tzZjxtsj;

    private String tzBsSlid;

    private String tzTjr;

    private Date tzTjsj;

    private String tzZhxgr;

    private Date tzZhxgsj;

    public String getTzFwqIp() {
        return tzFwqIp;
    }

    public void setTzFwqIp(String tzFwqIp) {
        this.tzFwqIp = tzFwqIp == null ? null : tzFwqIp.trim();
    }

    public String getTzJcfwqMs() {
        return tzJcfwqMs;
    }

    public void setTzJcfwqMs(String tzJcfwqMs) {
        this.tzJcfwqMs = tzJcfwqMs == null ? null : tzJcfwqMs.trim();
    }

    public String getTzCzxtLx() {
        return tzCzxtLx;
    }

    public void setTzCzxtLx(String tzCzxtLx) {
        this.tzCzxtLx = tzCzxtLx == null ? null : tzCzxtLx.trim();
    }

    public Integer getTzRwxhJg() {
        return tzRwxhJg;
    }

    public void setTzRwxhJg(Integer tzRwxhJg) {
        this.tzRwxhJg = tzRwxhJg;
    }

    public Integer getTzZdbxRws() {
        return tzZdbxRws;
    }

    public void setTzZdbxRws(Integer tzZdbxRws) {
        this.tzZdbxRws = tzZdbxRws;
    }

    public String getTzBeizhu() {
        return tzBeizhu;
    }

    public void setTzBeizhu(String tzBeizhu) {
        this.tzBeizhu = tzBeizhu == null ? null : tzBeizhu.trim();
    }

    public String getTzYxzt() {
        return tzYxzt;
    }

    public void setTzYxzt(String tzYxzt) {
        this.tzYxzt = tzYxzt == null ? null : tzYxzt.trim();
    }

    public Date getTzZjxtsj() {
        return tzZjxtsj;
    }

    public void setTzZjxtsj(Date tzZjxtsj) {
        this.tzZjxtsj = tzZjxtsj;
    }

    public String getTzBsSlid() {
        return tzBsSlid;
    }

    public void setTzBsSlid(String tzBsSlid) {
        this.tzBsSlid = tzBsSlid == null ? null : tzBsSlid.trim();
    }

    public String getTzTjr() {
        return tzTjr;
    }

    public void setTzTjr(String tzTjr) {
        this.tzTjr = tzTjr == null ? null : tzTjr.trim();
    }

    public Date getTzTjsj() {
        return tzTjsj;
    }

    public void setTzTjsj(Date tzTjsj) {
        this.tzTjsj = tzTjsj;
    }

    public String getTzZhxgr() {
        return tzZhxgr;
    }

    public void setTzZhxgr(String tzZhxgr) {
        this.tzZhxgr = tzZhxgr == null ? null : tzZhxgr.trim();
    }

    public Date getTzZhxgsj() {
        return tzZhxgsj;
    }

    public void setTzZhxgsj(Date tzZhxgsj) {
        this.tzZhxgsj = tzZhxgsj;
    }
}