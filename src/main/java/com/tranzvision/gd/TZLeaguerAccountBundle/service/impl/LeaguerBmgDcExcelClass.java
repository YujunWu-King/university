...
public class LeaguerBmgDcExcelClass {
	...
	public void tzGdDcBmbExcel(String runCntlId) {
		...
		try {
			...
			if (map != null) {
				...
				String oprIdList = (String) map.get("TZ_AUD_LIST");
				...
				/* 将需要导出报名表的编号存入数组 */
				String[] arrOprID = oprIdList.split(",");
				int row_count = arrOprID.length; /* 导出的数据的行数 */
				...
				for (int i = 0; i < row_count; i++) {
					...
					/*是否存在报名表*/
					List<Map<String, Object>> appInsIdList = null;
					String sqlGetBmbIns = "";
					...
					try {
						appInsIdList = jdbcTemplate.queryForList(sqlGetBmbIns,new Object[] { arrOprID[i] });
					} catch (Exception e) {
						appInsIdList = null;
					}
					
					if(appInsIdList!=null && appInsIdList.size() > 0){
						
						for (int i101 = 0; i101 < appInsIdList.size(); i101++) {
							String appInsId = appInsIdList.get(i101).get("TZ_APP_INS_ID")==null?"":String.valueOf(appInsIdList.get(i101).get("TZ_APP_INS_ID"));
							colum = 0;
							rowNum = rowNum + 1;
							Map<String, Object> mapData = new HashMap<String, Object>();
							mapData.put("id" + colum, String.valueOf(rowNum));
														...
														strAppFormFieldValue = jdbcTemplate
																.queryForObject(
																		sql1, new Object[] { strCodeTable,
																				Long.parseLong(appInsId), strAppFormField },
																		String.class);
													} catch (Exception e) {
													...
													if (appFormInfoView != null && !"".equals(appFormInfoView)&&isDisplayRecordValue) {
														...
														try {
															strAppFormFieldValue = jdbcTemplate.queryForObject(sql3,
																	new Object[] { Long.parseLong(appInsId) }, String.class);
														} catch (Exception e) {
															strAppFormFieldValue = "";
														}
													}
							...
							if(strAppclsId==null || "".equals(strAppclsId) || "null".equals(strAppclsId)){
								...
								if(mapClsInfo != null){
									try{
								        ...
								        Method method = myMethod.getClass().getMethod(strAppClsMethod, String.class,String.class, String.class);  
								        //调用这个方法，12是参数，String与之类似  
								        strAppFormFieldValues = (String) method.invoke(myMethod, "",arrOprID[i],appFormModalID);
									}catch (Exception e) {
										e.printStackTrace();
									} 
								}
