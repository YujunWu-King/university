...
public class tzOnlineAppUtility {
			...
			switch(strComMc){
				...
				case "DateComboBox":
				    ...
				    for (Object ObjLineNum : ListLineNum) {
				    	...
				    	numLineNum = mapObjLineNum.get("TZ_LINE_NUM") == null ? 0 : (int) mapObjLineNum.get("TZ_LINE_NUM");
				    	String sqlGetDate = "";
				    	sqlGetDate = "SELECT TZ_APP_S_TEXT FROM PS_TZ_APP_CC_VW2 WHERE TZ_APP_INS_ID = ? AND TZ_APP_TPL_ID = ? AND TZ_D_XXX_BH = ? AND TZ_XXX_BH LIKE ? AND TZ_XXX_NO = ? AND TZ_LINE_NUM = ?";
				    	strToToday = sqlQuery.queryForObject(sql, 
				    			new Object[] { numAppInsId,strTplId,strDxxxBh,strXxxBhLike + "%","com_todate",numLineNum }, "String");
				    	sqlGetDate = "SELECT TZ_APP_S_TEXT FROM PS_TZ_APP_CC_VW2 WHERE TZ_APP_INS_ID = ? AND TZ_APP_TPL_ID = ? AND TZ_D_XXX_BH = ? AND TZ_XXX_BH LIKE ? AND TZ_XXX_NO = ? AND TZ_LINE_NUM = ?";
				    	strStartDate = sqlQuery.queryForObject(sql, 
				    			new Object[] { numAppInsId,strTplId,strDxxxBh,strXxxBhLike + "%","com_startdate",numLineNum }, "String");
				    	sqlGetDate = "SELECT TZ_APP_S_TEXT FROM PS_TZ_APP_CC_VW2 WHERE TZ_APP_INS_ID = ? AND TZ_APP_TPL_ID = ? AND TZ_D_XXX_BH = ? AND TZ_XXX_BH LIKE ? AND TZ_XXX_NO = ? AND TZ_LINE_NUM = ?";
				    	strEndDate = sqlQuery.queryForObject(sql, 
				    			new Object[] { numAppInsId,strTplId,strDxxxBh,strXxxBhLike + "%","com_enddate",numLineNum }, "String");
				    	if("Y".equals(strToToday)){
				    		...
				    		if("".equals(strStartDate)||strStartDate==null||"".equals(strEndDate)||strEndDate==null){
				    			//校验失败
				    			returnMessage = this.getMsg(strXxxMc, strJygzTsxx);
				    		}
				    	}
			...
			switch(strComMc){
				...
				case "Check":
				    ...
				    for (Object ObjDhXxx : ListDhXxx) {
				    	...
				    	String strXxxBh2 = MapDhXxx.get("TZ_XXX_BH") == null ? "" : String.valueOf(MapDhXxx.get("TZ_XXX_BH"));
				    	...
				    	int numCheckCount = sqlQuery.queryForObject(sql, new Object[] { numAppInsId,strTplId,strXxxBh,strXxxBh2 }, "Integer");
	...
	//推荐信校验
	public String refLetterValidator(Long numAppInsId,String strTplId,String strXxxBh,String strXxxMc,String strComMc,
			int numPageNo,String strXxxRqgs,String strXxxXfmin,String strXxxXfmax,String strXxxZsxzgs,String strXxxZdxzgs,
			String strXxxYxsclx,String strXxxYxscdx,String strXxxBtBz,String strXxxCharBz,int numXxxMinlen,int numXxxMaxlen,
			String strXxxNumBz,int numXxxMin,int numXxxMax,String strXxxXsws,String strXxxGdgsjy,String strXxxDrqBz,
			int numXxxMinLine,String strTjxSub,String strJygzTsxx){
