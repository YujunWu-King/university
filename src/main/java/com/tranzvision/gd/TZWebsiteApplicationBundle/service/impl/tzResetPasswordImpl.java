...
public class tzResetPasswordImpl extends FrameworkImpl {
	...
	public String tzOther(String oprType, String strParams, String[] errorMsg) {
		...
		try {
			...
			if (oprType.equals("RESET_TJX_PASS")) {
				...
				// CSS样式载入:
				// System.out.println(paramMap.get("cssPath"));
				// if(page_stylecss==null){
				// page_stylecss=paramMap.get("cssPath").toString();
				// }
				...
				// equal："+tzLoginServiceImpl.getLoginedManagerOprid(request).equals(userId));

				// 2.3考生名称:
				// sql="SELECT TZ_REALNAME FROM PS_TZ_AQ_YHXX_TBL WHERE
				// OPRID=?";
				// String userName=jdbcTemplate.queryForObject(sql, new
				...
				// +"<p>您收到此封邮件是因为"+userName+"选择您作为他/她的推荐人，重置推荐信密码。</p>"
						+ "<p>请您复制以下地址，并使用IE8或以上版本浏览器打开，重置推荐信密码。<a href='" + url + "'> " + url + "</a></p>"
						+ "<p>如不是本人操作，请忽略本邮件。</p>";
				...
				// String
				// strJgid=tzLoginServiceImpl.getLoginedManagerOrgid(request);
				// String
	...
	public String tzGetHtmlContent(String strParams) {
		...
		try {
			...
			//<link href="/statics/css/website/orgs/sem/72/style_sem.css?v=8.448362827488486" rel="stylesheet" type="text/css" />
