 ...
 * @since 2015-12-18
 */
@Service("com.tranzvision.gd.TZSitePageBundle.service.impl.TzUrlTransServiceImpl")
public class TzUrlTransServiceImpl extends FrameworkImpl {
	...
	@Autowired
	private TzSetLoginPServiceImpl tzSetLoginPServiceImpl;
		...
		try {
			...
			if (null != strOrgId && !"".equals(strOrgId)) {
				...
				if (null != strSiteId && !"".equals(strSiteId)) {
					...
					return tzSetLoginPServiceImpl.tzGetHtmlContent(jacksonUtil.Map2json(mapParams));
				} else {
					return "站点尚未发布！";
				}
