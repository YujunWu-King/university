	...
	public String tzGetHtmlContent(String strParams) {
		...
		//try {
			...
			if(strSiteId!=null&&!"".equals(strSiteId)){
				String left1Sql = "SELECT A.TZ_ART_CONENT,C.TZ_AREA_PUBCODE FROM PS_TZ_ART_REC_TBL A INNER JOIN PS_TZ_LM_NR_GL_T B ON(A.TZ_ART_ID=B.TZ_ART_ID AND B.TZ_SITE_ID = ?) INNER JOIN PS_TZ_SITEI_AREA_T C ON (B.TZ_COLU_ID = C.TZ_COLU_ID AND C.TZ_SITEI_ID=B.TZ_SITE_ID AND C.TZ_AREA_POSITION = 'L1') LIMIT 1";
				...
				String left2Sql = "SELECT A.TZ_ART_CONENT,C.TZ_AREA_PUBCODE FROM PS_TZ_ART_REC_TBL A INNER JOIN PS_TZ_LM_NR_GL_T B ON(A.TZ_ART_ID=B.TZ_ART_ID AND B.TZ_SITE_ID = ?) INNER JOIN PS_TZ_SITEI_AREA_T C ON (B.TZ_COLU_ID = C.TZ_COLU_ID AND C.TZ_SITEI_ID=B.TZ_SITE_ID AND C.TZ_AREA_POSITION = 'L2') LIMIT 1";
