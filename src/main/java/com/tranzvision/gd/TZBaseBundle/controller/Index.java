...
public class Index {
	...
	public String dispatcher(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			case KJZY:
				// 主题编号;
				String sThemeID = CLASSJson.getString("themeID");
				// 获取框架资源信息;
				strComContent = gdKjInitService.getKJResources(request, response,
						sThemeID);
				break;
			// 主菜单;
			case ZCD:
			...
			case BQ:
				// 标签ID;
				String sCID = CLASSJson.getString("CID");
				// 默认内容名称;
				String sDefaultText = CLASSJson.getString("DText");
				// 获取控件标签;
				strComContent = gdKjComService.getFieldTag(request, response,
						sCID, sDefaultText, errMsgArr);
			...
			case ZJZY:
				...
				// 获取组件资源信息;
				strComContent = gdKjComService.getComResourses(request, response,
						sComID);
				break;
			// TransValue值;
			case TV:
				// 字段名称;
				String sFieldName = CLASSJson.getString("fieldName");
				// 获取该字段下的TransValue值;
			...
			case COMBOX:
				// 搜索表或视图名;
				String recname = CLASSJson.getString("recname");
				//搜索条件;
				String condition = CLASSJson.getString("condition");
				//搜索结果字段;
				String result = CLASSJson.getString("result");
			...
			case PROMPT:
				// 搜索表或视图名;
				String precname = CLASSJson.getString("recname");
				// 搜索条件;
				String pcondition = CLASSJson.getString("condition");
				// 搜索结果字段;
				String presult = CLASSJson.getString("result");
				// 搜索最大记录数;
				String maxRow = CLASSJson.getString("maxRow");
				...
				if (limit != null && !"".equals(limit)
						&& StringUtils.isNumeric(limit)) {
					numLimit = Integer.parseInt(limit);
				}

				if (strStart != null && !"".equals(strStart)
						&& StringUtils.isNumeric(strStart)) {
					numStart = Integer.parseInt(strStart);
				}

				// 根据搜索条件获取搜索表中的数据，供下拉框使用;
				strComContent = gdKjComService.getPromptSearchList(precname,
						pcondition, presult, maxRow, numLimit,
						numStart, errMsgArr);
				break;
			// 获取hardcode值;
			case HARDCODE:
			...
			default:
				...
				strComContent = gdKjComService.userRequestDispatcher(request,
						response, comID, sPageID, strOprType,
		...
		if ("HTML".equals(strOprType)) {
			if (!"1".equals(errorCode)) {
				...
				if (gdKjComService.isSessionValid(request)) {
					...
					//TODO;
					//strRetContent = this.getTimeoutHTML("TZGD_SQR_HTML_TIMEOUT");
					if (strRetContent == null
							|| "".equals(strRetContent)) {
						strRetContent = strErrorDesc;
					}
				}
			}
			...
			strRetContent = "{\"comContent\": " + strComContent
					...
					+ ",\"errdesc\": \""
					+ TZUtility.transFormchar(strErrorDesc)
					+ "\",\"timeout\": " + tmpTimeOutFlag
