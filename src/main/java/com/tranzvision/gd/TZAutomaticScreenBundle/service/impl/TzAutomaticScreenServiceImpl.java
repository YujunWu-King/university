...
public class TzAutomaticScreenServiceImpl extends FrameworkImpl {
	...
	public String tzQueryList(String strParams, int numLimit, int numStart, String[] errorMsg) {
		...
		try {
			...
			if (jacksonUtil.containsKey("type") && "expHistory".equals(jacksonUtil.getString("type"))) {
				// 排序字段
				String[][] orderByArr = new String[][] { { "TZ_STARTTIME", "DESC" } };

				// json数据要的结果字段;
				String[] resultFldArray = { "TZ_CLASS_BATCH", "TZ_DLZH_ID", "TZ_DR_TASK_DESC", "TZ_STARTTIME",
						"PROCESSINSTANCE", "PROCESS_STATUS", "TZ_REALNAME" };
				...
				if (obj != null && obj.length > 0) {
					...
					for (int i = 0; i < num; i++) {
						...
						if(!"".equals(scoreMsInsId)&& scoreMsInsId!=null){
							/* 面试打分项 */
						...
						zdbqVal = sqlQuery.queryForObject(zdbqSql, new Object[] { classId, rowList[1], rowList[2] },
								"String");
						...
						fmqdVal = sqlQuery.queryForObject(fmqdSql, new Object[] { classId, rowList[1], rowList[2] },
								"String");
	...
	private String setWeedOutByOrder(String strParams, String[] errorMsg) {
		...
		try {
			...
			if (!"".equals(classId) && classId != null && !"".equals(batchId) && batchId != null) {
				 ...
				 * sqlQuery.queryForList(setSql, new Object[]{ classId, batchId
				 * }); if(setList != null){ for(Map<String,Object> setMap:
				 * setList){ String appInsId = setMap.get("TZ_APP_INS_ID") ==
				 * null ? "":setMap.get("TZ_APP_INS_ID").toString();
				 * if(!"".equals(appInsId)){ sqlQuery.update(
				 * "update PS_TZ_CS_KS_TBL set TZ_KSH_CSJG='Y' where TZ_CLASS_ID=? and TZ_APPLY_PC_ID=? and TZ_APP_INS_ID=?"
					...
					if (outMc > 0) {
						...
						if (existsNum > 0) {
							if ((passNum + existsNum) > outNum) {
								...
								sqlQuery.update(
										"update PS_TZ_CS_KS_TBL set TZ_KSH_CSJG='N' where TZ_CLASS_ID=? and TZ_APPLY_PC_ID=? and TZ_KSH_PSPM=?",
										new Object[] { classId, batchId, outMc });
	...
	private String tzRunBatchProcess(String strParams, String[] errorMsg) {
		...
		try {
			...
			if (!"".equals(classId) && classId != null && !"".equals(batchId) && batchId != null) {
				...
				// 进程id;
				// int processinstance = sqlQuery.queryForObject("SELECT
				// TZ_JCSL_ID FROM TZ_JC_SHLI_T where TZ_YUNX_KZID = ? limit
				// 0,1", new Object[] { runCntlId },"Integer");
