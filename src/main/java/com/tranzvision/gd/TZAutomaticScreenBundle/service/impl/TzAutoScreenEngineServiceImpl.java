...
import com.tranzvision.gd.TZNegativeListInfeBundle.service.TzNegativeListInterface;
...
public class TzAutoScreenEngineServiceImpl{
	 ...
	 * @param classId 班级ID
	 * @param batchId 批次ID
	 * @param oprId 操作人员OPRID
	 * @throws TzException
	 * @throws TzSystemException 
	 */
	@SuppressWarnings("unchecked")
	public void autoScreen(String classId,String batchId, String oprId, BaseEngine logger) throws TzException, TzSystemException{
		...
		for(Map<String,Object> appInsMap: appInsList){
			...
			for(Map<String,Object> itemMap : itemsList){
				...
				String csDfgzId =  itemMap.get("TZ_ZDCSGZ_ID") == null ? "" 
						: itemMap.get("TZ_ZDCSGZ_ID").toString();
				...
				if(!"".equals(csDfgzCls)){
					...
					try{
						...
						e1.printStackTrace();
						//throw new TzException("自动打分规则定义不正确,规则编号："+csDfgzId+",错误原因："+e1.getMessage());
		...
		if(!"".equals(ksbqGroup) && ksbqGroup != null){
			List<Map<String,Object>> labelList = sqlQuery.queryForList(autoLabelSql, new Object[]{ orgId, ksbqGroup });
			for(Map<String,Object> labelMap : labelList){
				String labelId = labelMap.get("TZ_BIAOQ_ID").toString();
				String javaClass = labelMap.get("TZ_BIAOQZ_JAVA")==null ? "" : labelMap.get("TZ_BIAOQZ_JAVA").toString();
				...
				if(!"".equals(javaClass) && javaClass != null){
					try{
						...
						e2.printStackTrace();
						//throw new TzException("考生自动标签组："+ksbqGroup+",标签ID："+labelId+"定义有误。"+e2.getMessage());
		...
		if(!"".equals(fmqdGroup) && fmqdGroup != null){
			...
			for(Map<String,Object> fmqdMap : fmqdList){
				...
				String javaClass = fmqdMap.get("TZ_BIAOQZ_JAVA")==null ? "" : fmqdMap.get("TZ_BIAOQZ_JAVA").toString();
				
				if(!"".equals(javaClass) && javaClass != null){
					try{
						TzNegativeListInterface neGListObj = (TzNegativeListInterface) ctx.getBean(javaClass);
						neGListObj.makeNegativeList(classId, batchId, labelId);
					}catch(Exception e3){
						e3.printStackTrace();
						//throw new TzException("考生负面清单标签组："+ksbqGroup+",标签ID："+labelId+"定义有误。"+e3.getMessage());
						logger.logError("考生负面清单标签组："+ksbqGroup+",标签ID："+labelId+"定义有误。"+e3.getMessage());
					}
				}
			}
		}
