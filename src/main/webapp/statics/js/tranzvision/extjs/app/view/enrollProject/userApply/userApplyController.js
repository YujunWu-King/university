Ext.define('KitchenSink.view.enrollProject.userApply.userApplyController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.userApplyController',
    requires: [
        'KitchenSink.view.enrollProject.userApply.userApplyInfoPanel',
        'Ext.ux.IFrame',
    ...
    createUserInfoClass: function () {
        //是否有访问权限
        var pageResSet = TranzvisionMeikecityAdvanced.Boot.comRegResourseSet["TZ_UM_USERAPPLY_COM"]["TZ_UM_USERINFO_STD"];
    ...
    queryUser: function (btn) {
        //搜索条件过多，无法使用可配置搜索
        Ext.tzShowCFGSearch({
            cfgSrhId: 'TZ_UM_USERAPPLY_COM.TZ_UM_USERMG_STD.TZ_USER_APPLY_V',
            condition:
                {
    ...
    viewUser: function () {
        ...
        var win = this.lookupReference('userApplyInfoPanel');
        ...
        Ext.Ajax.request({
            ...
            success: function (response) {
                ...
                if (responseText.comContent.success == "true") {
                    ...
                    if (!win) {
                        className = 'KitchenSink.view.enrollProject.userApply.setPassword';
                        Ext.syncRequire(className);
                        ViewClass = Ext.ClassManager.get(className);
                        //新建类
                        win = new ViewClass();
    ...
    onSetPwdEnsure: function (btn) {
        ...
        var grid = btn.findParentByType("userApplyGL");
    ...
    saveAsStaAud: function () {
        ...
        //是否有访问权限
        var pageResSet = TranzvisionMeikecityAdvanced.Boot.comRegResourseSet["TZ_UM_USERAPPLY_COM"]["TZ_UM_AUDNEW_STD"];
    ...
    saveAsDynAud: function () {
        ...
        //是否有访问权限
        var pageResSet = TranzvisionMeikecityAdvanced.Boot.comRegResourseSet["TZ_UM_USERAPPLY_COM"]["TZ_UM_AUDDYN_STD"];
    ...
    exportExcelOrDownload: function (btn) {
        ...
        //是否有访问权限
        var pageResSet = TranzvisionMeikecityAdvanced.Boot.comRegResourseSet["TZ_UM_USERAPPLY_COM"]["TZ_EXP_EXCEL_STD"];
        if (pageResSet == "" || pageResSet == undefined) {
        ...
        if (!win) {
            ...
            ViewClass = Ext.ClassManager.get(className);
            //新建类
            // var modalID =btn.findParentByType('userApplyGL').child('form').getForm().findField('modalID').getValue();
            win = new ViewClass();
            this.getView().add(win);
        }
        ;
    ...
    exportSearchResultExcel: function (btn) {
        ...
        var OriSQL = "SELECT OPRID FROM TZ_USER_APPLY_V where TZ_JG_ID='" + JGID + "'";
    ...
    viewApplicationForm: function (grid, rowIndex, colIndex) {
        ...
        var classID = record.get("TZ_CLASS_ID");
        var oprID = record.get("OPRID");
        var appInsID = record.get("TZ_APP_INS_ID");
    ...
    printAppForm: function (obj, rowIndex, colIndex) {
        ...
        if (!obj.store) {
            ...
            appInsID = selList[0].get('TZ_APP_INS_ID');
    ...
    viewBmSch: function (grid, rowIndex, colIndex) {
        ...
        var pageResSet = TranzvisionMeikecityAdvanced.Boot.comRegResourseSet["TZ_UM_USERAPPLY_COM"]["TZ_UM_MSHLCH_STD"];
        if (pageResSet == "" || pageResSet == undefined) {
            Ext.MessageBox.alert('提示', '您没有访问或修改数据的权限');
            return;
        }
        ...
        // contentPanel.body.addCls('kitchensink-example');
        // className =
        // 'KitchenSink.view.enrollProject.userApply.userBmSchView';
        if (!Ext.ClassManager.isCreated(className)) {
            Ext.syncRequire(className);
        }
        ViewClass = Ext.ClassManager.get(className);
        ...
        cmp.on('afterrender', function () {
            ...
            var form = this.lookupReference('userApplyForm').getForm();
            //var ksdrInfoForm = this.lookupReference('userApplyForm').down('form[name=ksdrInfoForm]').getForm();
            var newCard = this.lookupReference('userApplyForm').down('form[name=kslcInfoForm]');
            //console.log(userApplyInfoPanel);
            //var btn=userApplyInfoPanel.down('by[name=save]');
           
            console.log(this.lookupReference('userApplyForm').down('form[name=kslcInfoForm]'));
...
//    	var appInsID = record.get("appInsId");
    ...
    sendEmlSelPers:function(btn) {
        ...
        for (var i = 0; i < checkLen; i++) {
            var oprID = selList[i].get('OPRID');
            var appInsID = selList[i].get('TZ_APP_INS_ID');
    ...
    viewApplicationForm1: function(grid, rowIndex,colIndex){
        ...
        var store = grid.getStore();
        var record = store.getAt(rowIndex);
        var classID=record.get("TZ_CLASS_ID");//暂时没有
        var oprID=record.get("OPRID");
        var appInsID=record.get("TZ_APP_INS_ID");
        console.log(appInsID);
        ...
        if(appInsID!=""){
            ...
            var win = new Ext.Window({
                ...
                maximized : true,
                controller:'userApplyController',//appFormClass
                classID :classID,
                oprID :oprID,
                appInsID : appInsID,
                gridRecord:record,
    ...
    sendSmsSelPers:function(btn) {
        ...
        var checkLen = selList.length;
        if (checkLen == 0) {
            Ext.Msg.alert(Ext.tzGetResourse("TZ_UM_USERAPPLY_COM.TZ_BMGL_STU_STD.prompt", "提示"), Ext.tzGetResourse("TZ_UM_USERAPPLY_COM.TZ_BMGL_STU_STD.youSelectedNothing", "您没有选中任何记录"));
            return;
        }
    ...
    printAppForm:function(obj,rowIndex,colIndex){
        ...
        if(!obj.store){
            ...
            appInsID = obj.getStore().getAt(rowIndex).get('TZ_APP_INS_ID');
        }
