Ext.define('KitchenSink.view.clueManagement.backgroundConfig.returnReason.defnReasonOfReturnController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.XSGLTHYYController',
    queryData:function(btn){
        /*var tzParams = '{"ComID":"TZ_GBYY_XSGL_COM","PageID":"TZ_GBYY_XSGL_STD","OperateType":"GS","comParams":{}}';
        Ext.tzLoad(tzParams,function(respData){*/
        Ext.tzShowCFGSearch({
            cfgSrhId: 'TZ_THYY_XSGL_COM.TZ_THYY_XSGL_STD.TZ_THYY_XSGL_V',
    ...
    addData:function(btn){
        ...
        this.showReturnReasonDetail(grid,"");
    },
    //编辑选中行的退回原因
    editCheckedData:function(btn){
        ...
        if (checkLen>=2) {
            ...
            this.showReturnReasonDetail(grid,reasonID);
        }
    },
    //编辑当前行的退回原因
    ...
    removeCurrentData:function(view,rowIndex){
        Ext.MessageBox.confirm('警告', '您确定要删除当前行的退回原因数据吗?', function(btnId){
            if(btnId == 'yes'){
    ...
    showReturnReasonDetail:function(grid,reasonID){
        //是否有访问权限
        var pageResSet = TranzvisionMeikecityAdvanced.Boot.comRegResourseSet["TZ_THYY_XSGL_COM"]["TZ_THYY_MODIF_STD"];
        ...
        if(className == "" || className == undefined){
            Ext.MessageBox.alert('提示', '未找到该功能页面对应的JS类，页面ID为：TZ_THYY_MODIF_STD，请检查配置。');
            return;
        }
        ...
        cmp.on('afterrender',function(panel){
        	...
        	var tzParams = '{"ComID":"TZ_THYY_XSGL_COM","PageID":"TZ_THYY_MODIF_STD",' +
    ...
    savReturnReasonGrid:function(grid,isClose){
        ...
        var tzParams = '{"ComID":"TZ_THYY_XSGL_COM","PageID":"TZ_THYY_XSGL_STD","OperateType":"U","comParams":{'+comParams+'}}';
...
//            panel.close();
    ...
    onModifiedSave:function(btn){
		...
		if(form.isValid()) {
            ...
            // var tzParams = '{"ComID":"TZ_THYY_XSGL_COM","PageID":"TZ_THYY_XSGL_STD","OperateType":"U","comParams":{'+comParams+'}}';
        	var tzParams = '{"ComID":"TZ_THYY_XSGL_COM","PageID":"TZ_THYY_XSGL_STD","OperateType":"U","comParams":{"update":['+Ext.JSON.encode(formParams)+']}}';
        	Ext.tzSubmit(tzParams,function(responseData){
            	form.setValues(responseData);
            	Ext.getCmp('tranzvision-framework-content-panel').down("grid[name =defnReasonOfReturnGrid]").store.reload();
        	},"",true,this);
		}
    },
    onModifiedEnsurn:function(btn){
		...
		if(form.isValid()) {
            ...
            // var comParams = "";
            // comParams = '"update":[' + Ext.JSON.encode(formParams) + "]";
            // var tzParams = '{"ComID":"TZ_THYY_XSGL_COM","PageID":"TZ_THYY_MODIF_STD","OperateType":"U","comParams":{'+comParams+'}}';
        	var tzParams = '{"ComID":"TZ_THYY_XSGL_COM","PageID":"TZ_THYY_MODIF_STD","OperateType":"U","comParams":{"update":['+Ext.JSON.encode(formParams)+']}}';
        	Ext.tzSubmit(tzParams,function(responseData){
            	Ext.getCmp('tranzvision-framework-content-panel').down("grid[name =defnReasonOfReturnGrid]").store.reload();
            	btn.findParentByType('panel').close();
        	},"",true,this);
