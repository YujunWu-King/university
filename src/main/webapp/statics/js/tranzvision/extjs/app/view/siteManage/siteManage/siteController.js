Ext.define('KitchenSink.view.siteManage.siteManage.siteController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.siteBasicB', 
	requires: [
    ...
    editSiteIntoByID:function(siteId){
		...
		cmp.on('afterrender',function(panel){
			...
			//参数
			var tzParams = '{"ComID":"TZ_GD_ZDGL_COM","PageID":"TZ_GD_ZDDY_STD","OperateType":"QF","comParams":{"siteId":"'+siteId+'"}}';
			//加载数据
			Ext.tzLoad(tzParams,function(responseData){
				...
				*/
				...
				//加载站点中的grid数据
				//站点模板集合	重新加载
				var siteModelgrid = panel.down('grid[name=siteModel]');
				tzStoreParams= '{"siteId":"'+siteId+'","queryID":"2"}';
				siteModelgrid.store.tzStoreParams = tzStoreParams;
				siteModelgrid.store.reload();
				
				//站点栏目集合	重新加载
				var siteColumngrid = panel.down('grid[name=siteColumn]');
				tzStoreParams= '{"siteId":"'+siteId+'","queryID":"3"}';
				siteColumngrid.store.tzStoreParams = tzStoreParams;
				siteColumngrid.store.reload();
				
				//站点区域集合	重新加载
				var siteAreagrid = panel.down('grid[name=siteArea]');
				tzStoreParams= '{"siteId":"'+siteId+'","queryID":"4"}';
				siteAreagrid.store.tzStoreParams = tzStoreParams;
				siteAreagrid.store.reload();
				
				//站点菜单集合	重新加载
				var siteMenugrid = panel.down('grid[name=siteMenu]');
				tzStoreParams= '{"siteId":"'+siteId+'","queryID":"5"}';
				siteMenugrid.store.tzStoreParams = tzStoreParams;
				siteMenugrid.store.reload();
				*/
			});
		});
