Ext.define('KitchenSink.view.tzLianxi.ldd.zxdc.zxdcMbController', {
   ...
   /*新增问卷模板*/
    addWjmb:function(btn){
        ...
        window.myWjmb=null;//目的是让他每次都执行下面的操作
        if (!window.myWjmb) {
            window.myWjmb = function(el) {
  ...
  setWjmb:function(view,rowindex){
    ...
    * */
    ...
    editWjmb:function(view,rowindex){
        ...
        var selRec = store.getAt(rowindex);
        var tplId = selRec.get("TZ_APP_TPL_ID");
        var pageResSet = TranzvisionMeikecityAdvanced.Boot.comRegResourseSet["TZ_ZXDC_MBGL_COM"]["TZ_ZXDC_EDIT_STD"];
        if (pageResSet == "" || pageResSet == undefined) {
            Ext.MessageBox.alert('提示', '您没有修改数据的权限');
            return;
        }
        var className = pageResSet["jsClassName"];
        if (className == "" || className == undefined) {
            Ext.MessageBox.alert('提示', '未找到该功能页面对应的JS类，页面ID为：TZ_ZXDC_EDIT_STD，请检查配置。');
            return;
        }
        var tzStoreParams = '{"edit":[{"ZXDC_TPL_ID":"' + this.TZ_APP_TPL_ID + '"}]}';
        var tzParams = '{"ComID":"TZ_ZXDC_MBGL_COM" ,"PageID":"TZ_ZXDC_EDIT_STD","OperateType":"HTML","comParams":' +tplId  + '}';
        Ext.tzSubmit(tzParams,
            function(jsonObject) {
               // store.reload();
            },"",true,this);
