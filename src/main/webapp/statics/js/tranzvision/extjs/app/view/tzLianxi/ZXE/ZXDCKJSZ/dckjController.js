Ext.define('KitchenSink.view.tzLianxi.ZXE.ZXDCKJSZ.dckjController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.dckj',
    queryKj:function(btn){
        Ext.tzShowCFGSearch({
            cfgSrhId: 'TZ_ZXDC_KJGL_COM.TZ_DCKJGL_LIST_STD.TZ_DCKJ_VW',
    ...
    addKj:function(btn){
        //是否有访问权限
        var pageResSet = TranzvisionMeikecityAdvanced.Boot.comRegResourseSet["TZ_ZXDC_KJGL_COM"]["TZ_DCKJGL_INFO_STD"];
        ...
        if(className == "" || className == undefined){
            Ext.MessageBox.alert('提示', '未找到该功能页面对应的JS类，页面ID为：TZ_DCKJGL_INFO_STD，请检查配置。');
            return;
        }
    ...
    editKj:function(obj,rowIndex){
        ...
        cmp.on('afterrender',function(panel){
            ...
            //参数
            var tzParams = '{"ComID":"TZ_ZXDC_KJGL_COM","PageID":"TZ_DCKJGL_INFO_STD","OperateType":"QF","comParams":{"kjID":"'+kjID+'"}}';
            //加载数据
    ...
    addKjJygz: function(btn) {
        ...
        if (formkj.isValid()) {
            // Ext.tzSetCompResourses("TZ_ZXDC_KJGL_COM");
            var pageResSet = TranzvisionMeikecityAdvanced.Boot.comRegResourseSet["TZ_ZXDC_KJGL_COM"]["TZ_DCKJJYGZ_STD"];
            if (pageResSet == "" || pageResSet == undefined) {
                Ext.MessageBox.alert('提示', '您没有修改数据的权限');
                return;
            }
            var className = pageResSet["jsClassName"];
            if (className == "" || className == undefined) {
                Ext.MessageBox.alert('提示', '未找到该功能页面对应的JS类，页面ID为：TZ_DCKJJYGZ_STD，请检查配置。');
                return;
            }
    ...
    editKjJygzByID:function(kjID,kjJygzID){
        ...
        var tzParams = '{"ComID":"TZ_ZXDC_KJGL_COM","PageID":"TZ_DCKJJYGZ_STD","OperateType":"QF","comParams":{"kjID":"'+kjID+'","kjJygzID":"'+kjJygzID+'"}}';
        //加载数据
        Ext.tzLoad(tzParams,function(responseData){
            var formData = responseData.formData;
            form.setValues(formData);
    ...
    getKjInfoParams: function(btn){
        ...
        //提交参数
        var tzParams = '{"ComID":"TZ_ZXDC_KJGL_COM","PageID":"TZ_DCKJGL_INFO_STD","OperateType":"U","comParams":{'+comParams+'}}';
        return tzParams;
    }
});
