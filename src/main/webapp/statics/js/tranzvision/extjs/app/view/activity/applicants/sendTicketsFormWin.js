Ext.define('KitchenSink.view.activity.applicants.sendTicketsFormWin', {
    extend: 'Ext.window.Window',
    xtype: 'sendMessagesType',
	reference: 'sendTicketsFormWindow',
	...
	items: [{
		...
		items:[{
            ...
            tpl: [
                '<tpl for=".">',
                    '<div class="send-type-item" style="float:left;margin:10px 20px 10px 10px;cursor:pointer">',
			...
			listeners:{
				itemclick:function(view, record, item, index, e){
					...
					var ignoreSent = win.child('form').getForm().getValues().filterSentBmr;
					if(ignoreSent != "Y") ignoreSent = "N";
					var actId = win.actId;
					var mbrArr = win.recArray;
					var checkLen = win.checkLen;
					var parmJson = "";
					var _bmrJson = ""
					
					parmData = '"activityId":"'+actId+'","ignoreSent":"'+ ignoreSent +'","checkLen":"'+checkLen+'","bmrIds":[';
					...
					if (record.data.type == 'E'){
						...
						Ext.tzLoad(parmJson,function(resp){
							...
							if(resp.count == 0){
								var alertDesc = "选择的";
								if(mbrArr.length == 0) alertDesc = "所有已报名的";
								Ext.Msg.alert("提示",alertDesc+"报名人已发送过邮件电子门票");
							}else{
								Ext.tzSendEmail({
									 //发送的电子门票邮件模板;
									 "EmailTmpName": ["TZ_HDBM_EWM"],
									 //创建的需要发送的听众ID;
						...
						Ext.tzLoad(parmJson,function(resp){
							...
							if(resp.count == 0){
								var alertDesc = "选择的";
								if(mbrArr.length == 0) alertDesc = "所有已报名的";
								Ext.Msg.alert("提示",alertDesc+"报名人已发送过短信电子门票");
							}else{
								Ext.tzSendSms({
								   //发送的短信模板;
								   //"SmsTmpName": ["TZ_HDBM_DZMP_M","TZ_HDBM_GXH_M"],
								   "SmsTmpName": ["TZ_HDBM_EWM"],
									//发送的听众;
									"audienceId": resp.audienceId
								});
							}
