				...
				'KitchenSink.view.enrollProject.userMg.userMgController',
				{
					...
					createUserInfoClass : function() {
						...
						if (className == "" || className == undefined) {
							Ext.MessageBox
									.alert('提示',
											'未找到该功能页面对应的JS类，页面ID为：TZ_UM_USERINFO_STD，请检查配置。');
							return;
						}
					...
					queryUser : function(btn) {
						...
						/*
						 * var pageResSet =
						 * TranzvisionMeikecityAdvanced.Boot.comRegResourseSet["TZ_UM_USERMG_COM"]["TZ_UM_MGSEARCH_STD"];
						 * if( pageResSet == "" || pageResSet == undefined){
						 * Ext.MessageBox.alert('提示', '您没有修改数据的权限'); return; }
						 * //该功能对应的JS类 var className =
						 * pageResSet["jsClassName"]; if(className == "" ||
						 * className == undefined){ Ext.MessageBox.alert('提示',
						 * '未找到该功能页面对应的JS类，页面ID为：TZ_UM_MGSEARCH_STD，请检查配置。');
						 * return; }
						 * 
						 * var win = this.lookupReference('userCfgSearch');
						 * 
						 * if (!win) { Ext.syncRequire(className); ViewClass =
						 * Ext.ClassManager.get(className); //新建类 win = new
						 * ViewClass(); this.getView().add(win); } win.show();
						 */
					},
					saveUserMgInfos : function(btn) {
						...
						for (var i = 0; i < removeRecs.length; i++) {
							if (removeJson == "") {
								removeJson = Ext.JSON
										.encode(removeRecs[i].data);
							} else {
								removeJson = removeJson + ','
										+ Ext.JSON.encode(removeRecs[i].data);
							}
						}
						var comParams = "";
						if (removeJson != "") {
						...
						// 提交参数
						var tzParams = '{"ComID":"TZ_UM_USERMG_COM","PageID":"TZ_UM_USERMG_STD","OperateType":"U","comParams":{'
								+ comParams + '}}';
						// 保存数据
					...
					viewUser : function() {
						// 选中行
						var selList = this.getView().getSelectionModel()
								.getSelection();
						// 选中行长度
						...
						// 参数
						var tzParams = '{"ComID":"TZ_UM_USERMG_COM","PageID":"TZ_UM_USERINFO_STD","OperateType":"QF","comParams":{"OPRID":"'
								+ OPRID + '"}}';
						// 加载数据
						Ext
								.tzLoad(
					...
					viewUserByBtn : function() {
						...
						cmp
								.on(
										...
										function() {
											var msgForm = this
													.lookupReference('userMgForm');
											var form = this.lookupReference(
													'userMgForm').getForm();
											var userInfoForm = this
													.lookupReference(
															'userMgForm')
													.down(
					...
					resetPassword : function() {
						...
						// 选中行
						var selList = comView.getSelectionModel()
								.getSelection();
						// 选中行长度
						var checkLen = selList.length;
						if (checkLen == 0) {
						...
						for (var i = 0; i < checkLen; i++) {
							if (editJson == "") {
								...
								editJson = editJson + ','
										+ Ext.JSON.encode(selList[i].data);
							}
						}
						comParams = '"data":[' + editJson + "]";
						// 提交参数
						var tzParams = '{"ComID":"TZ_UM_USERMG_COM","PageID":"TZ_UM_USERMG_STD","OperateType":"CHGPWD","comParams":{'
								+ comParams + '}}';
					...
					onSetPwdEnsure : function(btn) {
						...
						comParams = '"data":[' + editJson + "]";
						// 提交参数
						var tzParams = '{"ComID":"TZ_UM_USERMG_COM","PageID":"TZ_UM_USERMG_STD","OperateType":"PWD","comParams":{'
								+ pwdParams + "," + comParams + '}}';
						form.reset();
						Ext.tzSubmit(tzParams, function() {
							// 重置表单
					...
					/***********************************************************
					 * resetPassword: function(){ //选中行 var selList =
					 * this.getView().getSelectionModel().getSelection();
					 * //选中行长度 var checkLen = selList.length; if(checkLen == 0){
					 * Ext.Msg.alert("提示","请选择要重置密码的记录"); return; }else
					 * if(checkLen >1){ Ext.Msg.alert("提示","只能选择一条要重置密码的记录");
					 * return; }
					 * 
					 * var OPRID = selList[0].get("OPRID"); var JGID =
					 * Ext.tzOrgID; var tzChParams = '{"OPRID":"' + OPRID +
					 * '","JGID":"' + JGID + '"}'; var tzParams =
					 * '{"ComID":"TZ_UM_USERMG_COM","PageID":"TZ_UM_USERMG_STD","OperateType":"HTML","comParams":'+tzChParams+'}';
					 * Ext.Ajax.request({ //url:
					 * '/psc/TZDEV/EMPLOYEE/CRM/s/WEBLIB_GD_UM.TZ_GD_USERMG.FieldFormula.Iscript_ResetPassword',
					 * url:Ext.tzGetGeneralURL(), params: { tzParams: tzParams },
					 * success: function(response){
					...
					saveDataInfo : function() {
						...
						//grid数据
						var gridStore = win.lookupReference("viewAppGrid").getStore();
						var record = gridStore.getModifiedRecords();
						
						var array = new Array();
						for(var i=0;i<record.length;i++){
							array[i] = record[i].data;
						}
						formParams.updateStatus = array;
																	
						win.actType = "update";

						// 提交参数
						var tzParams = '{"ComID":"TZ_UM_USERMG_COM","PageID":"TZ_UM_USERINFO_STD","OperateType":"U","comParams":{"'
								+ win.actType
								+ '":[{"data":'
					...
					saveDataInfo2 : function() {
						...
						// 提交参数
						var tzParams = '{"ComID":"TZ_UM_USERMG_COM","PageID":"TZ_UM_MSHLCH_STD","OperateType":"U","comParams":{"'
								+ win.actType
								+ '":[{"data":'
								+ Ext.JSON.encode(formParams) + '}]}}';
						...
						Ext
								.tzShowPromptSearch({
									...
									srhresult : {
										...
										TZ_AUD_NAME : '听众名称'
									/*
									 * TZ_AUD_STAT: '听众状态', TZ_AUD_TYPE: '听众类型',
									 * TZ_AUD_MS: '描述', TZ_AUD_SQL: 'SQL'
									 */
									},
									multiselect : true,
									callback : function(selection) {
										if (selection.length > 0) {
											for (j = 0; j < selection.length; j++) {
												addAudirec = "";
												addAudirec = {
													"id" : selection[j].data.TZ_AUD_ID,
													"desc" : selection[j].data.TZ_AUD_NAME
												};
												arrAddAudience.push(addAudirec);
												arrAddAudiValue
														.push(selection[j].data.TZ_AUD_ID);
												arrAddNameValue
														.push(selection[j].data.TZ_AUD_NAME);
												/*
											...
											/*
											 * var pageResSet =
											 * TranzvisionMeikecityAdvanced.Boot.comRegResourseSet["TZ_UM_USERMG_COM"]["TZ_UM_AUDNEW_STD"]; //
											 * console.log(pageResSet); if(
											 * pageResSet == "" || pageResSet ==
											 * undefined){
											 * Ext.MessageBox.alert('提示',
											 * '您没有修改数据的权限'); return; }
											 * //该功能对应的JS类 var className =
											 * pageResSet["jsClassName"]; //
											 * console.log(className);
											 * 
											 * if(className == "" || className ==
											 * undefined){
											 * Ext.MessageBox.alert('提示',
											 * '未找到该功能页面对应的JS类，页面ID为：TZ_UM_AUDNEW_STD，请检查配置。');
											 * return; }
											 * 
											 * var win =
											 * this.lookupReference('pageRegWindow');
											 * 
											 * if (!win) { className =
											 * 'KitchenSink.view.enrollProject.userMg.userMgNewAud';
											 * Ext.syncRequire(className);
											 * ViewClass =
											 * Ext.ClassManager.get(className);
											 * //新建类 win = new ViewClass(); //
											 * this.getView().add(win); }
											 * win.actType = "update";
											 */
											 ...
											 * var audMS = arrAddMsValue; var
											 * audSQL = arrAddSQLValue;
											 * 
											 */
											...
											var tzParams2 = '{"ComID":"TZ_AUD_COM","PageID":"TZ_AUD_LIST_STD","OperateType":"U","comParams":{'
													+ comParamsOPRID + '}}';
											 ...
											 * 
											 * //参数 var tzParams =
											 * '{"ComID":"TZ_AUD_COM","PageID":"TZ_AUD_NEW_STD","OperateType":"QF","comParams":{"audId":"'+audId+'","audName":"'+audName+'","audStat":"'+audStat+'","audType":"'+audType+'","audMS":"'+audMS+'","audSQL":"'+audSQL+'"}}';
											 * //页面注册信息表单
											 * 
											 * var form =
											 * win.child("form").getForm();
											 * 
											 * var gridStore
											 * =win.child("form").child("grid").getStore();
											 * var tzStoreParams =
											 * '{"cfgSrhId":"TZ_AUD_COM.TZ_AUD_NEW_STD.PS_TZ_AUDCY_VW","condition":{"TZ_AUD_ID-operator":
											 * "01","TZ_AUD_ID-value": "'+
											 * audId+'"}}';
											 * 
											 ...
											 * 
											 * 
											 * win.show(); var SmsGroupDetForm =
											 * btn.findParentByType('smsGroupDet').child('form');
											 * var
											 * storereceive=SmsGroupDetForm.child('tagfield[reference="receverTagField"]').getStore();
											 ...
											 * SmsGroupDetForm.child('toolbar[reference=receverToolbar]').child('button[reference=clearAllBtn]').removeCls('x-item-disabled
											 * x-btn-disabled');
											 */
										}
									}
					...
					saveAsStaAud : function() {
						...
						// 参数
						var tzParams = '{"ComID":"TZ_AUD_COM","PageID":"TZ_AUD_NEW_STD","OperateType":"QF","comParams":{"audId":"'
								+ audId
								+ '","audName":"'
								+ audName
								+ '","audStat":"'
						...
						var gridStore = win.child("form").child("grid")
								.getStore();
						var tzStoreParams = '{"cfgSrhId":"TZ_AUD_COM.TZ_AUD_NEW_STD.PS_TZ_AUDCY_VW","condition":{"TZ_AUD_ID-operator": "01","TZ_AUD_ID-value": "'
								+ audId + '"}}';
					...
					savePageRegInfo : function(win, view) {
						...
						var gridStore = win.child("form").child("grid")
								.getStore();
						var selList = win.child("form").child("grid")
						...
						var tzParams2 = '{"ComID":"TZ_AUD_COM","PageID":"TZ_AUD_NEW_STD","OperateType":"U","comParams":{'
								+ comParams + '}}';
						// console.log(tzParams2);
						// 保存数据
						if (comParams != "") {
						...
						// 表单数据
						var comParamsALL = '"update":[{"typeFlag":"FORM","data":'
						...
						for (var i = 0; i < updateRecs.length; i++) {
							if (updateJson == "") {
								// updateJson =
								// Ext.JSON.encode(updateRecs[i].data);
								updateJson = '{"typeFlag":"GRID","data":'
										+ Ext.JSON.encode(updateRecs[i].data)
										+ '}';
								// editJson =
								// '{"typeFlag":"PAGE","data":'+Ext.JSON.encode(mfRecs[i].data)+'}';
						...
						if (updateJson != "") {
							...
							comParamsALL = comParamsALL + ',"update":['
									+ updateJson + "]";
						...
						// var tzParams3 =
						// '{"ComID":"TZ_AUD_COM","PageID":"TZ_AUD_NEW_STD","OperateType":"U","comParams":{'+comParams3+'}}';
						// console.log("tzParams3:"+tzParams3);
						// 保存数据
						/*
						 * if(comParams3!=""){
						 * Ext.tzSubmit(tzParams3,function(){
						 * gridStore.reload(); },"",true,this); }
						 */

						// 提交参数
						// var tzParams =
						// '{"ComID":"TZ_AUD_COM","PageID":"TZ_AUD_NEW_STD","OperateType":"U","comParams":{"'+win.actType+'":['+Ext.JSON.encode(formParams)+']}}';
						var tzParams = '{"ComID":"TZ_AUD_COM","PageID":"TZ_AUD_NEW_STD","OperateType":"U","comParams":{'
								+ comParamsALL + '}}';
						// var tzParams =
						// '{"ComID":"TZ_AUD_COM","PageID":"TZ_AUD_NEW_STD","OperateType":"QF","comParams":{"audId":"'+audId+'","audName":"'+audName+'","audStat":"'+audStat+'","audType":"'+audType+'"}}';

						console.log(tzParams);

						// var tzStoreParams =
						// '{"comID":"'+formParams["comID"]+'"}';
										...
										buttons : [
												{
													text : Ext
															.tzGetResourse(
																	"TZ_BMGL_BMBSH_COM.TZ_BMGL_STU_STD.audit",
																	"审批"),
													iconCls : "send",
												...
												},
												{
													text : Ext
															.tzGetResourse(
																	"TZ_BMGL_BMBSH_COM.TZ_BMGL_STU_STD.close",
																	"关闭"),
													iconCls : "close",
													handler : function() {
														win.close();
				    ...
				    selectProvince: function(btn){
				        ...
				        Ext.tzShowPromptSearch({
				            ...
				            multiselect: false,
				            callback: function(selection){
				            	form.findField("lenProvince").setValue(selection[0].data.descr);
				            	
				                /*form.findField(oprID).setValue(selection[0].data.TZ_CARD_NO);
				                var cardId=selection[0].data.TZ_CARD_NO;
				                var tzParams = '{"ComID":"TZ_LIBCARD_MAN_COM","PageID":"TZ_CARD_INFO_STD","OperateType":"QF","comParams":{"cardId":"'+cardId+'"}}';
				                //加载数据
				                Ext.tzLoad(tzParams,function(responseData){
