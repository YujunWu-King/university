Ext.define('KitchenSink.view.judgesManagement.judgeAccount.judgeAccController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.judgeAccController',
    createClass: function () {
        //是否有访问权限
        var pageResSet = TranzvisionMeikecityAdvanced.Boot.comRegResourseSet["TZ_JUDGE_ACC_COM"]["TZ_JUDINFO_STD"];
        ...
        if (className == "" || className == undefined) {
            Ext.MessageBox.alert('提示', '未找到该功能页面对应的JS类，页面ID为：TZ_JUDINFO_STD，请检查配置。');
            return;
        }
...
//		this.judgeAccInfoHandler(accNum,oprid,"update");
	...
	judgeAccInfoHandler: function(accNum, oprid, type){
		...
		var params = '{"ComID":"TZ_JUDGE_ACC_COM","PageID":"TZ_JUDINFO_STD","OperateType":"QU","comParams":{}}';
...
//				var tzParams = '{"ComID":"TZ_JUDGE_ACC_COM","PageID":"TZ_JUDACC_GL_STD","OperateType":"QF","comParams":{"accountNo":"'+accNum+'"}}';
				var tzParams = '{"ComID":"TZ_JUDGE_ACC_COM","PageID":"TZ_JUDACC_GL_STD","OperateType":"QF","comParams":{"accountNo":"'+accNum+'","orgId":"'+Ext.tzOrgID+'"}}';
...
//					form.setValues(responseData.formData);
	...
	onSave: function(btn){
		...
		var tzParams = '{"ComID":"TZ_JUDGE_ACC_COM","PageID":"TZ_JUDACC_GL_STD","OperateType":"U","comParams":{';
		//删除数据行
...
//			if (checked.length > 0){
				if (psd1 == psd2){
					...
					var tzParams = '{"ComID":"TZ_JUDGE_ACC_COM","PageID":"TZ_JUDINFO_STD","OperateType":"U","comParams":{'+comParams+'}}';
					Ext.tzSubmit(tzParams,function(respData){
						if (actType == "add"){
							win.actType = "update";
							form.findField("accountNo").setReadOnly(true    );
	...
	searchJudgeAcc: function(btn){
		Ext.tzShowCFGSearch({
			cfgSrhId: 'TZ_JUDGE_ACC_COM.TZ_JUDACC_GL_STD.TZ_PWZHGL_VW',
			condition:
            {
