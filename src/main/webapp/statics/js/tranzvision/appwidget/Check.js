            ...
            if(SurveyBuild.accessType == "M"){
                for (var i in data.option) {
                	 if(SurveyBuild._readonly){
                		 if(data["option"][i]["checked"] == "Y"){
                			 e += '<li style="margin: 5px auto 10px;">';
                             e += '		<input style="padding: 0; display: inline-block; width: 0.7rem; height: 0.7rem;  margin-right: 5px; vertical-align: middle; box-sizing: border-box; -moz-box-sizing: border-box; -webkit-box-sizing: border-box;"type="checkbox" name="' + data.itemId + '" class="' + (data["option"][i]["other"] == "Y" ? "sur_other_box" : "") + '" instanceId="' + i + '" id="o' + data.itemId + data["option"][i]["code"] + '" ' + (data["option"][i]["checked"] == "Y" ? "checked='checked'": "") + ' value="' + data["option"][i]["code"]+'"/>';
                             e += '<label for ="o' + data.itemId + data["option"][i]["code"] + '" ' + (data["option"][i]["checked"] == "Y" ? "checked='checked'": "")+'" style="display: inline-block;font-size: 0.58rem;line-height: 0.7rem;color: #666; vertical-align: middle;">'+data["option"][i]["txt"]+'</label>'
                	...
                	e += '<li style="margin: 5px auto 10px;">';
//					e += '	<div class="check-box ' + (data["option"][i]["checked"] == "Y" ? "checkedBox": "") + '">';
                    e += '		<input style="padding: 0; display: inline-block; width: 0.7rem; height: 0.7rem;  margin-right: 5px; vertical-align: middle; box-sizing: border-box; -moz-box-sizing: border-box; -webkit-box-sizing: border-box;"type="checkbox" name="' + data.itemId + '" class="' + (data["option"][i]["other"] == "Y" ? "sur_other_box" : "") + '" instanceId="' + i + '" id="o' + data.itemId + data["option"][i]["code"] + '" ' + (data["option"][i]["checked"] == "Y" ? "checked='checked'": "") + ' value="' + data["option"][i]["code"]+'"/>';
                    e += '<label for ="o' + data.itemId + data["option"][i]["code"] + '" ' + (data["option"][i]["checked"] == "Y" ? "checked='checked'": "")+'" style="display: inline-block;font-size: 0.58rem;line-height: 0.7rem;color: #666; vertical-align: middle;">'+data["option"][i]["txt"]+'</label>'
                    //					e += '</div>';
//					e += ;
        ...
        $inputBox.parents(".check-box").on('click', function () {
            ...
            if (scheck) {
                ...
                if (data["option"][meid]["other"] == "Y") {
                    $("#other" + data.itemId).css("display", "inline");
                }
            } else {
...
//		}
        ...
        console.log(NumErrorMsg);

        console.log(data.isRequire );
        console.log(data.minSelect)
        console.log(data.maxSelect)
        if (data.isRequire == "Y") {
