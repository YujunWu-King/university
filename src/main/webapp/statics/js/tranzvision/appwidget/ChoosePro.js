...
+ 开发人：曹阳														++
+ 开发日期：2017-01-16	
+ 2018-1-17  xzx修改（用于选择项目和班级开始日期）						++
=====================================================================*/
SurveyBuild.extend("ChoosePro", "baseComponent", {
	itemName: "项目选择",
	title:"项目选择",
	isDoubleLine:"Y",
	isSingleLine: "Y",
	...
	children:{
		"bmrClass": {
			"instanceId": "bmrClass",
			"itemId": "CC_Project",
			"itemName": MsgSet["CHOOSE_COURSE"],
			"title": MsgSet["CHOOSE_COURSE"],
			"orderby": 1,
			"value": "",
			"StorageType": "S",
			"wzsm": "",
	...
	_getHtml : function(data,previewmode){
	     ...
	     if(previewmode){
	    	 ...
	    	 if($("#ClassId").length > 0){
	             params = '{"ComID":"TZ_ONLINE_REG_COM","PageID":"TZ_ONREG_OTHER_STD","OperateType":"EJSON","comParams":{"OType":"PROINFO","CLASSID":' + classid + '}}';
	                $.ajax({
	                    type: "get",
	                    dataType: "JSON",
	                    data:{
			 ...
			 if(SurveyBuild.accessType == "M"){
				 ...
				 if(SurveyBuild._readonly || $("#ClassId").length <= 0){
			    	 ...
			    	 if(parseInt(child.bmrClass.zssm) <=0){
			    		 e += '<a href="#" style="display:none" id="'+data["itemId"]+child.bmrClass.itemId+'_Btnselect">'+MsgSet["CHAGE_DATE"]+'</a></p>';
					 }else{
						 e += '<a href="#" style="display: inline;margin-left: 30px;" id="'+data["itemId"]+child.bmrClass.itemId+'_Btnselect">'+MsgSet["CHAGE_DATE"]+'</a></p>';
					 }
				 }
				 ...
				 if(SurveyBuild._readonly){
					 ...
					 // 只读模式(批次显示)
					 if(batchId !='' && child.bmrBatch.wzsm!=''){
						 e += '<div class="item">';
				 ...
				 e += '	<div class="input-list-info left"><span class="red-star">' + (data.isRequire == "Y" ? "*": "") + '</span>' + MsgSet["CHOOSE_COURSE"] + '</div>';
				 ...
				 if(SurveyBuild._readonly || $("#ClassId").length <= 0){
					 ...
					 if(parseInt(child.bmrClass.zssm) <=0){
						 e += '	<div class="input-list-text left" ><span id="'+data["itemId"]+child.bmrClass.itemId+'_SPAN">' + child.bmrClass.wzsm + '</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="#" style="display:none" id="'+data["itemId"]+child.bmrClass.itemId+'_Btnselect">'+MsgSet["CHAGE_DATE"]+'</a></div>';
					 }else{
						 e += '	<div class="input-list-text left" ><span id="'+data["itemId"]+child.bmrClass.itemId+'_SPAN">' + child.bmrClass.wzsm + '</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="#" id="'+data["itemId"]+child.bmrClass.itemId+'_Btnselect">'+MsgSet["CHAGE_DATE"]+'</a></div>';
					 }
				 }

				 // e += '	<div class="input-list-suffix left"></div>'; 样式覆盖“选择上课日期”不能点击
				 ...
				 if(SurveyBuild._readonly){
					 ...
					 if(array.length != 0){
						 e += '<div class="input-list" id="div1">';
						 e += '	<div class="input-list-info left"><span class="red-star">' + (data.isRequire == "Y" ? "*": "") + '</span>' + MsgSet["INTERVIEW_BATCH"] + '</div>';
				 ...
				 // c += '<div class="main_inner_content">';有背景图
				 c += '<div class="">';
				 c += e;
				 c += '</div>';
				...
				e += '	<span class="edu_item_label">您申请的课程：</span>';
