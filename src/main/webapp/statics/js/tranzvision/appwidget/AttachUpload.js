...
//                c += '</div>';
                ...
                c += '			</span>';
                c += '			<div class="ncsc-upload-btn-p">'+MsgSet["CHOOSE_ATTACH"]+'<img src="' + TzUniversityContextPath + '/statics/js/onlineSurvey/images/m/upload.png"></div>';
                c += '		</a>';
                c += ' </div>';
                c += '</div>';
                c += '<span >'+(data.suffixUrl != "" ? "<a href='" + data.suffixUrl + "'>" : "") + (data.suffix != "" ? data.suffix : "") + (data.suffixUrl != "" ? "</a>" : "</span>");
                //附件列表
                c += '	<div id="'+ data.itemId +'_attList" class="finder_list " style="display:'+(children.length>0 && children[0].sysFileName != "" ? "":"none")+'"  >';
                //显示已上传附件
                for(var i=0; i<children.length; i++){
                    if(children[i].sysFileName != ""){
                        c += '<li><a onclick="SurveyBuild.downLoadFile(this,\''+data.instanceId+'\')" file-index="'+ children[i].orderby +'"><p>'+ children[i].viewFileName +'</p></a>';
                        c += '	<img src="' + TzUniversityContextPath + '/statics/js/onlineSurvey/images/m/det.png"  title="'+MsgSet["DEL"]+'" onclick="SurveyBuild.deleteFile(this,\''+data.instanceId+'\')">&nbsp;'+MsgSet["DEL"]+'<div>';
