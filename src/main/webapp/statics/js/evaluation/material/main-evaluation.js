...
function createSubStatisticsCharts(chartDataArray,chartStore)
{
	...
	if(fsChart1 != null)
	{
		retObject = {title:chartDataArray['ps_fszb_mc'] + '指标统计曲线图',layout:'fit',items:fsChart1};
	}
...
function getApplicantListColumns(jsonObject)
{
	...
	columnList.push({text:"评审时间",width:120,align:'left',sortable:true,resizable:false,dataIndex:"ps_ksh_dt"});
												...
												try
												{
													...
													else
													{
														...
														else
														{
                                                            ...
                                                            //lw tzEObject.baokaoPcID = jsonObject['ps_pc_id'];
															...
															//tzEObject.applicantInterviewID = jsonObject['ps_ksh_msid'];
...
{	
  ...
  var grid = Ext.create('Ext.grid.Panel', {
      ...
      fbar: [
      				...
      				'->',
					'总体提交状态：<span id="ps_kslb_submtall_'+ jsonObject['ps_class_id']+"_"+jsonObject['ps_bkpc_id'] +'" style="margin-right:10px;font-weight:bold;">【' + ps_kslb_submtall_status +'】</span>',
      				{
      					text:'提交全部考生数据',
      					tooltip:'单击此按钮提交当前评审批次。',
      					handler : function()
      				...
      				},
      				{
      					...
      					handler : function()
      										{
                                                //lw var cls_pc_id = jsonObject['ps_class_id']+"_"+jsonObject['ps_pc_id'];
                                                //var cls_pc_id = jsonObject['ps_class_id']+"_"+jsonObject['ps_bkpc_id'];
...
function createStatisticsArea(batchId,jsonObject)
{
	...
	/*创建评委评分平均分统计图表*/
	var tjCharts1 = createStatisticsChart(jsonObject,null,1020);
	if(tjCharts1 != null)
	{
	...
	/*创建评委评分分布统计图表*/
	var tjCharts2 = createStatisticsCharts(jsonObject,null,1020);
	if(tjCharts2 != null)
	{
...
function refreshBatchDataByBatchId(jsonObject,dataIndexName,dataIndexValue)
{
	...
	try
	{
		var pwFenbuGridStore = rfObject1['items']['items'][1]['items']['items'][2].getStore();
