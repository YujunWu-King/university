...
(function($) {
	...
	// DOM elements
	overlay, closeBtn,bigBtn,smallBtn, image, prevBtn, nextBtn, leftBtn, rightBtn, bottom, caption, nav, number,
		...
		$(document.body).append(
			$([
				overlay = $('<div id="pbOverlay" />').click(close).append(
					//closeBtn = $('<div style="z-index:99999; position:absolute;" id="pbCloseBtn" />')[0]
				)[0],
				bigBtn = $('<div style="z-index:99999; position:fixed;" id="bigBtn" />').click(bigImage)[0],
				smallBtn = $('<div style="z-index:99999; position:fixed;" id="smallBtn" />').click(smallImage)[0],
				closeBtn = $('<div style="z-index:99999; position:fixed;" id="pbCloseBtn" />').click(close)[0],
	...
	*/
	...
	function showImage(noAnim) {
		...
		var mw = win.width() - options.margins, mh = win.height() - options.margins, size = 1.0;
		//console.log("宽度:"+mw);
		//console.log("高度:"+mh);
	...
	function bigImage(e) {
		$(zoomBtn).addClass(zoomed);
		var big = currentSize + 0.2;
		return resizeImage(big);
	}
	...
	function smallImage(e) {
		...
		$(zoomBtn).addClass(zoomed);
		if(currentSize>0.2){
			small = currentSize - 0.2;
		}else{
			small = 0.01;
		}
		return resizeImage(small);
	}
