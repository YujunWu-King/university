...
SurveyBuild.extend("workExperience", "baseComponent", {
    ...
    _getHtmlOne: function(data, rownum) {
        ...
        for (var i in child) {
            ...
            if (SurveyBuild._readonly) {
                ...
                works += '<img id="' + data.itemId + child[i].work_bdate["itemId"] + '_Btn" src="/tranzvision/images/calendar.png" style="position:relative;left:' + (child[i].work_tonow["value"] == "Y" ? "-40px": "-168px") + ';cursor:pointer;">';
                works += '<img id="' + data.itemId + child[i].work_edate["itemId"] + '_Btn" src="/tranzvision/images/calendar.png" style="position:relative;left:-61px;cursor:pointer;' + (child[i].work_tonow["value"] == "Y" ? "display:none;": "") + '">';
                ...
                if (j != 0) {
                    ...
                    works += '  <img src="/onlineReg/images/del.png" width="15" height="15">&nbsp;' + MsgSet["DEL"];
                    works += '</div>';
                }
                works += '</div>';
            }
    ...
    _eventbind: function(data) {
        ...
        $startDt.formValidator({
            tipID: data["itemId"] + children[len - 1].work_edate["itemId"] + 'Tip',
            onShow: "",
            onFocus: "&nbsp;",
            onCorrect: "&nbsp;"
        });
        if (data.isRequire == "Y") {
            $startDt.inputValidator({
                min: '1',
                onError: children[len - 1].work_bdate["itemName"] + MsgSet["REQUIRE"]
            });
        };
        ...
        if (data.isRequire == "Y") {
            $endDt.inputValidator({
                min: '1',
                onError: children[len - 1].work_edate["itemName"] + MsgSet["REQUIRE"]
            });
        }
        ...
        $today.formValidator({tipID: data["itemId"] + children[len - 1].work_edate["itemId"] + 'Tip',onShow: "",onFocus: "&nbsp;",onCorrect: "&nbsp;"}).functionValidator({
            fun: function(val, elem) {
                var startTime = $startDt.val();
                var endTime = $endDt.val();
                var todayDt = new Date();
        ...
        if (data.isRequire == "Y") {
            $company.inputValidator({
                min: '1',
                onError: children[len - 1].work_comp["itemName"] + MsgSet["REQUIRE"]
            });
        }

        $zhiwu.formValidator({tipID: data["itemId"] + children[len - 1].work_zhiwu["itemId"] + 'Tip',onShow: "",onFocus: "&nbsp;",onCorrect: "&nbsp;"});
        if (data.isRequire == "Y") {
            $zhiwu.inputValidator({
                min: '1',
                onError: children[len - 1].work_zhiwu["itemName"] + MsgSet["REQUIRE"]
            });
        }
    }
})