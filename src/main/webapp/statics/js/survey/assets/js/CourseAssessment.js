/**
 * Created by WRL on 2016/5/26.
 */
SurveyBuild.extend("CourseAssessment", "baseComponent", {
    itemName: "课程评估",
    title: "课程评估",
    StorageType: "T",
    qCode: "",//题号
        ...
        if (previewmode) {
            ...
            for (var i in data.child) {
                ...
                for (var j in data.option) {
                    s += '<td><img src="../img/read-radio.gif"></td>';
                }
                s += '</tr>';
            }
    ...
    _eventbind: function(data) {
        $.each(data.child,
            function(i, ch) {
                ...
                $inputBox.formValidator({
                    tipID: (data["itemId"] + 'Tip'),
                    onShow: "&nbsp;",
                    onFocus: "&nbsp;",
                    onCorrect: "&nbsp;"
                });
                if (!allowEmpty) {
                    $inputBox.inputValidator({
                        min: 1,
                        onError: errorMsg
                    });
                }
