SurveyBuild.extend("EduExperience", "baseComponent", {
	...
	_getHtmlOne: function(data, rownum) {
		...
		for (var i in child) {
			...
			if (SurveyBuild._readonly) {
				...
				if (j != 0) {
					edus += '	<div onclick="SurveyBuild.deleteFun(this);" class="main_inner_content_del_bmb"><img width="15" height="15" src="/onlineReg/images/del.png">' + MsgSet["DEL"] + '</div>';
				}
				edus += '</div>';
			}
	...
	_eventbind: function(data) {
		var children = data.children,len = children.length;
		...
		if (data.isRequire == "Y") {
			...
			$studyStage.inputValidator({min: '1',onError: children[len - 1].EduExper1["itemName"] + MsgSet["REQUIRE"]});

			$startDt.formValidator({tipID: data["itemId"] + children[len - 1].EduExper2["itemId"] + 'Tip',onShow: "",onFocus: "&nbsp;",onCorrect: "&nbsp;"});
			$startDt.inputValidator({min: '1',onError: children[len - 1].EduExper2["itemName"] + MsgSet["REQUIRE"]});
			...
			$endDt.inputValidator({min: '1',onError: children[len - 1].EduExper2["itemName"] + MsgSet["REQUIRE"]});
			...
			$school.inputValidator({min: '1',onError: children[len - 1].EduExper5["itemName"] + MsgSet["REQUIRE"]});

			$major.formValidator({tipID: data["itemId"] + children[len - 1].EduExper6["itemId"] + 'Tip',onShow: "",onFocus: "&nbsp;",onCorrect: "&nbsp;"});
			$major.inputValidator({min: '1',onError: children[len - 1].EduExper6["itemName"] + MsgSet["REQUIRE"]});

			$byzNo.formValidator({tipID: data["itemId"] + children[len - 1].EduExper7["itemId"] + 'Tip',onShow: "",onFocus: "&nbsp;",onCorrect: "&nbsp;"});
			$byzNo.inputValidator({min: '1',onError: children[len - 1].EduExper7["itemName"] + MsgSet["REQUIRE"]	});

			$xwzNo.formValidator({tipID: data["itemId"] + children[len - 1].EduExper8["itemId"] + 'Tip',onShow: "",onFocus: "&nbsp;",onCorrect: "&nbsp;"});
			$xwzNo.inputValidator({min: '1',onError: children[len - 1].EduExper8["itemName"] + MsgSet["REQUIRE"]	});
		}
