...
(function($) {
	$.fn.listnav = function(options) {
		...
		return this.each(function() {
					...
					if (opts.includeAll) $('.all', $letters).addClass('ln-selected'); 
						...
						for (var i = ((opts.includeNums) ? 0 : 1); i < letters.length; i++) { 
							if (counts[letters[i]] > 0) { 
								firstClick = true; 
								$('.' + letters[i], $letters).slice(0, 1).click(); break;
							} 
						} 
					} 
				} 
			...
			function addClasses() {
				...
				//var filterSelector = opts.filterSelector;
				$($list).children().each(function() {					
					$this = $(this); 
					firstChar = '', str = $.trim($this.text()).toLowerCase(); 
					if (str != '') {
						if (hasPrefixes) { 
							spl = str.split(' '); 
			...
			function bindHandlers() {
				if (opts.showCounts) { 
					$wrapper.mouseover(function() { setLetterCountTop(); }); 
				}
				if (opts.showCounts) { 
					...
					$('a', $letters).mouseout(function() { $letterCount.hide(); }); 
				}
	...
	$.fn.listnav.defaults = { initLetter: '', includeAll: true, incudeOther: false, includeNums: true, flagDisabled: true, noMatchText: 'No matching entries', showCounts: true, cookieName: null, onClick: null, prefixes: [] ,filterSelector:''};
})(jQuery);
